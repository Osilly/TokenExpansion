...
class VisionTransformerExpansion(VisionTransformer):
    ...
    def forward_features(self, x):
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            for i, layer in enumerate(self.blocks):
                # ---------------------#
                if i == 1:
                    x, _ = self.token_select.token_select(x)
                # ---------------------#
                x = layer(x)
...
def _create_deit(variant, pretrained=False, distilled=False, **kwargs):
    ...
    model_cls = VisionTransformerDistilled if distilled else VisionTransformerExpansion
    # model_cls = VisionTransformerExpansion
