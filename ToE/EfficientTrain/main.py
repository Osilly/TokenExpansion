...
def get_args_parser():
    parser = argparse.ArgumentParser(
        "(EfficientTrain) ConvNeXt training and evaluation script for image classification",
        add_help=False,
    )
    ...
    #####  Added by EfficientTrain  #####
    parser.add_argument(
        "--end_epoch", default=300, type=int, metavar="N", help="end epoch"
    )
    parser.add_argument(
        "--base_input_size", default=224, type=int, help="basic image input size"
    )
...
def main(args):
    ...
    print("Number of training iter per epoch = %d" % num_training_steps_per_epoch)
    ...
    for epoch in range(args.start_epoch, args.end_epoch):
        ...
        if args.sparse_eval:
            ...
            model_without_ddp.token_select.sparse_inference = False
        # ---------------------#
        ...
        if data_loader_val is not None:
            ...
            if args.model_ema and args.model_ema_eval:
                # ---------------------#
                model_ema.ema.token_select.update_current_stage(epoch)
