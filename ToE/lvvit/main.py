...
def main():
    ...
    try:
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            # ---------------------#
            if args.distributed:
                model.module.token_select.update_current_stage(epoch)
            else:
                model.token_select.update_current_stage(epoch)
            # ---------------------#

            # ---------------------#
            if args.distributed:
                model.module.token_select.sparse_inference = True
            else:
            ...
            if model_ema is not None and not args.model_ema_force_cpu:
                # ---------------------#
                model_ema.module.token_select.update_current_stage(epoch)

                if args.sparse_eval:
                    model_ema.module.sparse_inference = True
                else:
                    model_ema.module.sparse_inference = False
                # ---------------------#
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        total_train_time.update(end_train_time - start_train_time, 1)
        # ---------------------#
    ...
    return OrderedDict(
        [
            ("loss", losses_m.avg),
            # ---------------------#
            ("total_train_time", total_train_time.sum)
            # ---------------------#
        ]
    )
