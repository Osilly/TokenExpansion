...
class LV_ViT_Expansion(nn.Module):
    ...
    def forward_tokens(self, x):
        ...
        # ---------------------#
        temp = torch.zeros_like(x).to(device=x.device)
        # ---------------------#
        for i, blk in enumerate(self.blocks):
            # ---------------------#
            if i == 1:
                x, select_index = self.token_select.token_select(x)
            # ---------------------#
            x = blk(x)
        ...
        # ---------------------#
        # restore to the original token set
...
def _create_lvvit_expansion(variant, pretrained=False, **kwargs):
    model_cls = LV_ViT_Expansion
...
def lvvit(pretrained=False, **kwargs):
    ...
    model = _create_lvvit_expansion(
        "lvvit", pretrained=pretrained, **dict(model_args, **kwargs)
    )
    return model
...
def lvvit_t(pretrained=False, **kwargs):
    ...
    model = _create_lvvit_expansion(
        "lvvit_t", pretrained=pretrained, **dict(model_args, **kwargs)
    )
    return model
...
def lvvit_s(pretrained=False, **kwargs):
    ...
    model = _create_lvvit_expansion(
        "lvvit_s", pretrained=pretrained, **dict(model_args, **kwargs)
    )
    return model
...
def lvvit_m(pretrained=False, **kwargs):
    ...
    model = _create_lvvit_expansion(
        "lvvit_m", pretrained=pretrained, **dict(model_args, **kwargs)
    )
    return model
...
def lvvit_l(pretrained=False, **kwargs):
    ...
    model = _create_lvvit_expansion(
        "lvvit_l", pretrained=pretrained, **dict(model_args, **kwargs)
    )
    return model
