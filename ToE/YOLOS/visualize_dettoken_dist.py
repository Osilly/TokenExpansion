...
parser.add_argument(
    "--visjson", default="", type=str, help="the json path to visualize"
)
parser.add_argument(
    "--cococlsjson",
    default="vis_token_dist/coco-valsplit-cls-dist.json",
    type=str,
    help="the json path to visualize",
)
...
fig, axs = plt.subplots(
    ncols=10, nrows=1, figsize=(22, 2), facecolor="white", tight_layout=True
)
for index, ax in enumerate(axs):
    for index, row in tokens_df[index].iterrows():
        ...
        ax.scatter(
            100 * row["cx"],
            100 * row["cy"],
            c=size_colors[int(row["color_idx"])],
            cmap="brg",
            s=5,
            alpha=0.3,
            linewidth=0,
...
g = sns.lineplot(
    data=merge_tokens_cls_df, x="category", y="#objects", label="Det-Tok dist"
)
...
val_2 = sns.scatterplot(
    data=vis_valcls_df, x="category", y="#objects", palette=["red"], color="red"
)
