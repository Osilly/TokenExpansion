...
from .token_select import TokenSelect
...
class VisionTransformerExpansion(nn.Module):
    ...
    def forward_features(self, x):
        ...
        for i in range(len((self.blocks))):
            # ---------------------#
            if i == 1:
                x_without_det_token = x[:, : x.shape[1] - det_token.shape[1], :]
                x_with_det_token = x[:, x.shape[1] - det_token.shape[1] :, :]
                x_without_det_token, select_index = self.token_select.token_select(
                    x_without_det_token
                )
                x = torch.cat([x_without_det_token, x_with_det_token], dim=1)
                # print(x_without_det_token.shape, x_with_det_token.shape, x.shape)
            # ---------------------#
            ...
            if self.has_mid_pe:
                if i < (self.depth - 1):
                    ...
                    if select_index is not None:
                        temp_mid_pos_embed_i = temp_mid_pos_embed[i]
                        temp_mid_pos_embed_without_det_token = temp_mid_pos_embed_i[
                            :, : temp_mid_pos_embed_i.shape[1] - det_token.shape[1], :
                        ]
                        temp_mid_pos_embed_with_det_token = temp_mid_pos_embed_i[
                            :, temp_mid_pos_embed_i.shape[1] - det_token.shape[1] :, :
                        ]
                        # print(
                        #     temp_mid_pos_embed_without_det_token.shape,
                        #     temp_mid_pos_embed_with_det_token.shape,
                        # )
                        temp_mid_pos_embed_without_det_token = (
                            temp_mid_pos_embed_without_det_token.gather(
                                dim=1, index=select_index
                            )
                        )
                        # print(temp_mid_pos_embed_without_det_token.shape)
                        reduced_temp_mid_pos_embed = torch.cat(
                            [
                                temp_mid_pos_embed_without_det_token,
                                temp_mid_pos_embed_with_det_token,
                            ],
                            dim=1,
                        )
                        x = x + reduced_temp_mid_pos_embed
                        # print(x.shape, reduced_temp_mid_pos_embed.shape)
                    else:
...
def tiny(pretrained=None, **kwargs):
    model = VisionTransformerExpansion(
        patch_size=16,
        embed_dim=192,
        depth=12,
        num_heads=3,
